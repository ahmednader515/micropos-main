...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch sales and purchases for capital calculation
    ...
    doc.text('تقرير رأس المال', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    doc.setFontSize(14)
    doc.setFont('Amiri', 'bold')
    doc.text('ملخص رأس المال', margin, currentY, { isInputRtl: true })
    currentY += 10
    ...
    currentY += 8
    doc.text(`تكلفة البضائع المباعة: ${costOfGoodsSold.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 8
    doc.text(`إجمالي الربح: ${grossProfit.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 8
    doc.text(`صافي الربح: ${netProfit.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 15
    
    // Generate PDF buffer
    const pdfBuffer = Buffer.from(doc.output('arraybuffer'))
    
    const filename = `capital_report_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating capital report PDF:', error)
