...
export async function GET(request: NextRequest) {
  try {
    ...
    doc.text('تقرير بحركة الرصيد الأفتتاحي و النقد للمورد', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    const filename = `supplier_opening_balance_${supplier.name}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating supplier opening balance report PDF:', error)
