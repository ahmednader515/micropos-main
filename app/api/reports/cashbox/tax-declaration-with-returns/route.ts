...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch sales and returns for tax declaration
    ...
    doc.text('تقرير بالاقرار الضريبي مع المرتجع', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    const totalReturns = returns.reduce((sum: number, returnSale: Sale & { items: any[] }) => sum + parseFloat(returnSale.totalAmount.toString()), 0)
    const netSales = totalSales - totalReturns
    const vatRate = 0.15 // 15% VAT rate
    const vatAmount = netSales * vatRate
    
    doc.setFontSize(14)
    doc.setFont('Amiri', 'bold')
    doc.text('البيانات الضريبية', margin, currentY, { isInputRtl: true })
    currentY += 10
    ...
    currentY += 8
    doc.text(`إجمالي المرتجع: ${totalReturns.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 8
    doc.text(`صافي المبيعات: ${netSales.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 8
    doc.text(`نسبة ضريبة القيمة المضافة: ${(vatRate * 100).toFixed(1)}%`, margin, currentY, { isInputRtl: true })
    currentY += 8
    doc.text(`مبلغ ضريبة القيمة المضافة: ${vatAmount.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 15
    
    // Generate PDF buffer
    const pdfBuffer = Buffer.from(doc.output('arraybuffer'))
    
    const filename = `tax_declaration_with_returns_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating tax declaration with returns PDF:', error)
