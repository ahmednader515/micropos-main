...
export default function SalesPage() {
  ...
  const [successModalCustomer, setSuccessModalCustomer] = useState<Customer | null>(null)
  const [successModalItems, setSuccessModalItems] = useState<SaleItem[]>([])
  ...
  const renderCategoryGrid = (categories: any[], isSubcategoryView = false) => {
    return categories.map((category) => (
      <button
        ...
        style={{
          height: 'calc((100vh - 12rem) / 9)' // 1/9 of available space (3 rows in 1/3 section)
        }}
      >
  ...
  const handleSendToWhatsApp = async () => {
    if (!successModalCustomer || !successModalCustomer.phone) {
      showNotification('error', 'لا يوجد رقم هاتف للعميل')
      return
    }
    ...
    try {
      ...
      if (invoiceNumber === 'عرض سعر') {
        // For price display, use the same logic as print button
        const tempSaleData = {
          invoiceNumber: 'عرض سعر',
          totalAmount: checkoutForm.total,
          paidAmount: checkoutForm.paid,
        ...
        response = await fetch('/api/pdf/invoice', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        // For regular invoices, fetch from database like print button
        const saleResponse = await fetch(`/api/sales?invoiceNumber=${invoiceNumber}`)
        if (!saleResponse.ok) {
          throw new Error('فشل في العثور على الفاتورة')
        }

        const saleData = await saleResponse.json()
        if (!saleData.sales || saleData.sales.length === 0) {
          throw new Error('الفاتورة غير موجودة')
        }

        const sale = saleData.sales[0]
        response = await fetch(`/api/pdf/invoice?id=${sale.id}`)
      }
      ...
      const cleanPhone = successModalCustomer.phone.replace(/[\s\-\(\)]/g, '')
          ...
          style={{
            height: showInlineProductSelection 
              ? (showCategoryView && showInlineProductSelection
                  ? 'calc((100vh - 12rem) / 3 + 2rem)' // Both sections visible - 1/3 of available space + extra for header
                  : 'calc((100vh - 8rem) / 2 + 2rem)' // Only products section visible - 1/2 of available space + extra for header
                )
          ...
          <div className="fixed bottom-12 left-0 right-0 z-30" dir="rtl">
                  ...
                  <div className="grid grid-cols-3 gap-1">
                    {getFilteredProducts().map(product => (
                        <button
                          ...
                          style={{
                            height: showCategoryView 
                              ? 'calc((100vh - 12rem) / 9)' // 1/9 of available space (3 rows in 1/3 section)
                              : 'calc((100vh - 8rem) / 6)', // 1/6 of available space (3 rows in 1/2 section)
            ...
            {showCategoryView && (
              <div 
                ...
                style={{ height: 'calc((100vh - 12rem) / 3)' }} // 1/3 of available space
              >
                <div className="p-1">
                  <div className="grid grid-cols-3 gap-1">
                ...
                {invoiceNumber === 'عرض سعر' ? (
                  ...
                  <>
                    ...
                    <div className="flex gap-3">
                      ...
                      {successModalCustomer && successModalCustomer.phone && (
                        <button
