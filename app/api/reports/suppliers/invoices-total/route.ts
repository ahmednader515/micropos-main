...
export async function GET(request: NextRequest) {
  try {
    ...
    doc.text('تقرير بالفواتير لمورد - اجمالي', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    const filename = `supplier_invoices_total_${supplier.name}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating supplier invoices total report PDF:', error)
