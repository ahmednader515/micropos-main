...
interface SupplierAccountPopupProps {
  isVisible: boolean
  onClose: () => void
  startDate: string
  endDate: string
...
export default function SupplierAccountPopup({ isVisible, onClose, startDate, endDate }: SupplierAccountPopupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      a.href = url
      a.download = `supplier_account_${supplierName.trim()}_${new Date().toISOString().split('T')[0]}.pdf`
      a.click()
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-gray-900">كشف حساب مورد</h2>
            <button
              onClick={onClose}
