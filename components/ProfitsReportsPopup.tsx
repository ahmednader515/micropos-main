...
interface ProfitsReportsPopupProps {
  isVisible: boolean
  onClose: () => void
...
export default function ProfitsReportsPopup({ isVisible, onClose, onReportSelect, startDate, endDate }: ProfitsReportsPopupProps) {
  ...
  const generatePDF = async (reportType: string, additionalData?: any) => {
    try {
      ...
      const res = await fetch(`/api/reports/profits?${params.toString()}`, { method: 'GET' })
      if (!res.ok) {
      ...
      const reportTitles: { [key: string]: string } = {
        'by-product': 'profits_report_by_product',
        'total': 'profits_report_total',
        'by-invoice': 'profits_report_by_invoice',
        'category': `profits_report_category_${additionalData?.category?.name || ''}`,
        'by-category': `profits_report_by_category_${additionalData?.categoryName || ''}`,
        'customer': `profits_report_customer_${additionalData?.customer?.name || additionalData?.customerName || ''}`
      }
      
      a.download = `${reportTitles[reportType] || 'profits_report'}.pdf`
      a.click()
  ...
  return (
    <>
      {/* Main Profits Reports Popup */}
      {isVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" dir="rtl">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-gray-900">تقرير بالأرباح</h2>
              <button
                onClick={onClose}
            ...
            <div className="space-y-2">
              ...
              >
                <span className="text-sm">تقرير بالارباح حسب الصنف</span>
              ...
              <button
                onClick={() => handleReportClick('total')}
                className="w-full text-right px-3 py-2 text-gray-900 hover:bg-gray-50 rounded-lg transition-colors duration-200 flex items-center justify-between group"
              >
                <span className="text-sm">تقرير بالمبيعات و الارباح - اجمالي</span>
                <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </button>
              ...
              <button
                ...
                className="w-full text-right px-3 py-2 text-gray-900 hover:bg-gray-50 rounded-lg transition-colors duration-200 flex items-center justify-between group"
              >
                <span className="text-sm">تقرير بالارباح حسب رقم الفاتورة</span>
                <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </button>
              ...
              <button
                ...
                className="w-full text-right px-3 py-2 text-gray-900 hover:bg-gray-50 rounded-lg transition-colors duration-200 flex items-center justify-between group"
              >
                <span className="text-sm">تقرير بالارباح حسب العميل</span>
                <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </button>
              ...
              <button
                ...
                className="w-full text-right px-3 py-2 text-gray-900 hover:bg-gray-50 rounded-lg transition-colors duration-200 flex items-center justify-between group"
              >
                <span className="text-sm">تقرير بالارباح لتصنيف</span>
                <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </button>
