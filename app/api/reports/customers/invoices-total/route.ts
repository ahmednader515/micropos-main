...
export async function GET(request: NextRequest) {
  try {
    ...
    doc.text('تقرير بالفواتير لعميل - اجمالي', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    doc.setFontSize(12)
    doc.setFont('Amiri', 'normal')
    doc.text(`إجمالي عدد الفواتير: ${totalInvoices}`, margin, currentY, { isInputRtl: true })
    currentY += 8
    ...
    doc.setFontSize(14)
    doc.setFont('Amiri', 'bold')
    doc.text('التفصيل حسب طريقة الدفع', margin, currentY, { isInputRtl: true })
    currentY += 10
    ...
    const filename = `customer_invoices_total_${sanitizedName}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating customer invoices total report PDF:', error)
    return new NextResponse(JSON.stringify({ 
