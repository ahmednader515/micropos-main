...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Purchase request PDF generated successfully, size:', pdfBuffer.length, 'bytes')
    
    return new NextResponse(pdfBuffer, {
      status: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="purchase_request_${new Date().toISOString().split('T')[0]}.pdf"`,
      },
    })
    ...
    return NextResponse.json(
      { error: 'حدث خطأ أثناء توليد طلب الشراء' },
      { status: 500 }
    )
  }
}
