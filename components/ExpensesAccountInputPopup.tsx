...
interface ExpensesAccountInputPopupProps {
  isVisible: boolean
  onClose: () => void
  title: string
  apiEndpoint: string
...
export default function ExpensesAccountInputPopup({ isVisible, onClose, title, apiEndpoint, startDate, endDate, startTime, endTime }: ExpensesAccountInputPopupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      a.href = url
      a.download = `${title.replace(/\s+/g, '_')}_${accountName.trim()}_${new Date().toISOString().split('T')[0]}.pdf`
      a.click()
