...
export async function GET(request: NextRequest) {
  try {
    ...
    doc.text('كشف حساب مورد', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    doc.text(`البريد الإلكتروني: ${supplier.email || 'غير محدد'}`, margin, margin + 65, { isInputRtl: true })
    
    // Date range
    const dateRange = `من ${start.toLocaleDateString('ar-SA')} إلى ${end.toLocaleDateString('ar-SA')}`
    doc.text(`الفترة: ${dateRange}`, margin, margin + 75, { isInputRtl: true })
    ...
    if (supplier.purchases.length > 0) {
      ...
      supplier.purchases.forEach((purchase) => {
        ...
        doc.text(Number(purchase.totalAmount).toFixed(2), margin + 80, currentY, { isInputRtl: true })
        doc.text(paymentMethodNames[purchase.paymentMethod] || purchase.paymentMethod, margin + 120, currentY, { isInputRtl: true })
    ...
    if (payments.length > 0) {
      ...
      doc.text('التاريخ', margin + 40, currentY, { isInputRtl: true })
      doc.text('طريقة الدفع', margin + 80, currentY, { isInputRtl: true })
      ...
      payments.forEach((payment) => {
        ...
        doc.text(paymentMethodNames[payment.paymentMethod] || payment.paymentMethod, margin + 80, currentY, { isInputRtl: true })
    ...
    const filename = `supplier_account_${supplier.name}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating supplier account report PDF:', error)
