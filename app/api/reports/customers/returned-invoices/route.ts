...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get returned invoices (sales with CANCELLED status) for the date range
    const returnedInvoices = await prisma.sale.findMany({
      where: {
    ...
    doc.text('تقرير بالفواتير المرتجع لعميل', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    // Calculate totals
    const totalReturnedInvoices = returnedInvoices.length
    const totalReturnedAmount = returnedInvoices.reduce((sum: number, invoice: Sale) => sum + Number(invoice.totalAmount), 0)
    ...
    doc.setFontSize(12)
    doc.setFont('Amiri', 'normal')
    doc.text(`عدد الفواتير المرتجعة: ${totalReturnedInvoices}`, margin, currentY, { isInputRtl: true })
    currentY += 8
    doc.text(`إجمالي المبلغ المرتجع: ${totalReturnedAmount.toFixed(2)} ج.م`, margin, currentY, { isInputRtl: true })
    currentY += 15
    ...
    if (returnedInvoices.length > 0) {
      doc.setFontSize(14)
      doc.setFont('Amiri', 'bold')
      doc.text('تفاصيل الفواتير المرتجعة', margin, currentY, { isInputRtl: true })
      currentY += 10
      ...
      doc.text('تاريخ الإرجاع', margin + 160, currentY, { isInputRtl: true })
      ...
      returnedInvoices.forEach((invoice: Sale & { items: any[] }) => {
        ...
        doc.text(invoice.updatedAt.toLocaleDateString('ar-SA'), margin + 160, currentY, { isInputRtl: true })
      ...
      doc.setFontSize(12)
      doc.setFont('Amiri', 'normal')
      doc.text('لا توجد فواتير مرتجعة في الفترة المحددة', margin, currentY, { isInputRtl: true })
    }
    ...
    const filename = `customer_returned_invoices_${sanitizedName}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating customer returned invoices report PDF:', error)
    return new NextResponse(JSON.stringify({ 
