...
export async function GET(request: NextRequest) {
  try {
    ...
    const reportTitles: { [key: string]: string } = {
      'by-product': 'تقرير بالارباح حسب الصنف',
      'total': 'تقرير بالمبيعات و الارباح - اجمالي',
      'by-invoice': 'تقرير بالارباح حسب رقم الفاتورة',
      'category': 'تقرير بالارباح لتصنيف',
      'by-category': 'تقرير بالارباح لتصنيف',
      'customer': 'تقرير بالارباح حسب العميل'
    }
    
    const title = reportTitles[reportType || 'by-product'] || 'تقرير بالأرباح'
    ...
    sales.forEach(sale => {
      ...
      sale.items.forEach(item => {
        ...
        const itemProfit = (Number(item.price) - Number(item.product.costPrice)) * item.quantity
        totalCost += itemCost
    ...
    currentY += 8
    doc.text(`إجمالي التكلفة: ${totalCost.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 8
    doc.text(`إجمالي الأرباح: ${totalProfit.toFixed(2)} ريال`, margin, currentY, { isInputRtl: true })
    currentY += 8
    ...
    doc.text('المبيعات', margin + 120, currentY, { isInputRtl: true })
    doc.text('الأرباح', margin + 150, currentY, { isInputRtl: true })
    ...
    sales.forEach((sale, index) => {
      ...
      doc.text(saleProfit.toFixed(2), margin + 150, currentY, { isInputRtl: true })
    ...
    const filename = `profits_report_${reportType || 'by_product'}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating profits report PDF:', error)
