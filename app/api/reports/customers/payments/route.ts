...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get customer payments (payments with type PAY) for the date range
    ...
    doc.text('تقرير بسندات الصرف لعميل', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    doc.setFontSize(12)
    doc.setFont('Amiri', 'normal')
    doc.text(`عدد سندات الصرف: ${totalPayments}`, margin, currentY, { isInputRtl: true })
    currentY += 8
    ...
    if (payments.length > 0) {
      doc.setFontSize(14)
      doc.setFont('Amiri', 'bold')
      doc.text('تفاصيل سندات الصرف', margin, currentY, { isInputRtl: true })
      currentY += 10
      ...
      payments.forEach((payment: Payment) => {
        ...
        doc.text(payment.id.substring(0, 8), margin + 30, currentY, { isInputRtl: true })
        ...
        doc.text(payment.notes || '-', margin + 160, currentY, { isInputRtl: true })
      ...
      doc.setFontSize(12)
      doc.setFont('Amiri', 'normal')
      doc.text('لا توجد سندات صرف في الفترة المحددة', margin, currentY, { isInputRtl: true })
    }
    ...
    const filename = `customer_payments_${sanitizedName}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating customer payments report PDF:', error)
    return new NextResponse(JSON.stringify({ 
