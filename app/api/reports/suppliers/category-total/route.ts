...
export async function GET(request: NextRequest) {
  try {
    ...
    doc.text('تقرير اجمالي حسب الصنف لمورد', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    const filename = `supplier_category_total_${sanitizedName}_${new Date().toISOString().split('T')[0]}.pdf`
    ...
    console.error('Error generating supplier category total report PDF:', error)
