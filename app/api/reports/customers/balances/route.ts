...
export async function GET() {
  try {
    ...
    const htmlContent = `
      ...
      <body>
        <div class="header">
          <h1 class="title">الأرصدة الافتتاحية و المبالغ النقدية للعملاء</h1>
        </div>
        ...
        <table class="table">
          ...
          <tbody>
            ${customers.map(customer => {
              ...
              const creditClass = hasAmount > 0 ? 'credit' : 'zero'
              const debitClass = oweAmount > 0 ? 'debit' : 'zero'
              ...
              return `
                <tr>
                  ...
                  <td class="${creditClass}">${hasAmount.toFixed(2)}</td>
                  <td class="${debitClass}">${oweAmount.toFixed(2)}</td>
                </tr>
              `
            }).join('')}
          </tbody>
