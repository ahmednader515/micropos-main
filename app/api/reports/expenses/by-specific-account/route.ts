...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!accountName) {
      return new NextResponse(JSON.stringify({ error: 'اسم الحساب مطلوب' }), {
        status: 400,
    ...
    doc.text('تقرير بالمصروفات لحساب', pageWidth / 2, margin + 10, { align: 'center', isInputRtl: true })
    ...
    doc.setFontSize(14)
    doc.setFont('Amiri', 'bold')
    doc.text('بيانات الحساب', margin, margin + 30, { isInputRtl: true })
    
    doc.setFontSize(12)
    doc.setFont('Amiri', 'normal')
    doc.text(`اسم الحساب: ${accountName}`, margin, margin + 45, { isInputRtl: true })
    ...
    currentY += 8
    doc.text(`إجمالي المبلغ: ${totalExpenses.toFixed(2)} ج.م`, margin, currentY, { isInputRtl: true })
    currentY += 15
    ...
    console.error('Error generating expenses by specific account report PDF:', error)
